...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <!-- Start Page Content -->
        <!-- ============================================================== -->
        <form id="add_category" class="form-horizontal" method="POST" action="{{ url('/admin/quiz/update') }}"
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" name="passing_score"class="form-control" id="title"
                                        placeholder="Passing Score" value="{{ $quiz->passing_score }}">
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                        ...
                                        <table id="zero_config" class="table table-striped table-bordered">
                                            ...
                                            <tbody>
                                                @foreach ($quiz->questions as $key => $question)
                                                    <tr>
                                                        <td>{{ $key + 1 }}</td>
                                                        <td><input type="hidden" name="Question[]"
                                                                value="{{ $question->id }}">{{ $question->question }}</td>
                                                        <td>
                                                            <button type="button"
                                                                class="btn btn-sm btn-clean btn-icon btn-icon-md"
                                                                data-toggle="modal" id="{{ $question->id }}"
                                                                value="{{ $question->title }}"
                                                                data-target="#deleteError" data-target="#deleteQuestion">
                                ...
                                <div class="border-top">
                                    <div class="card-body text-right">
                                        <input type="submit" class="btn btn-primary" value="Update Question"></button>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-success"
                        onclick="addQuestion(document.getElementById('question-select').options[document.getElementById('question-select').selectedIndex])"><i
                            class="feather icon-plus-circle mr-25"></i>Add</button>
        ...
        function addQuestion(selected) {
            ...
            if (selected.value != "") {
                ...
                action = '<button type="button" class="btn btn-sm btn-clean btn-icon btn-icon-md" data-toggle="modal" ' +
                    'data-target="#deleteQuestion" id="' + selected.value + '" value="' + selected.text +
